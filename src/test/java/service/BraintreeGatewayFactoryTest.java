...
public class BraintreeGatewayFactoryTest {
	...
	public void loadsCorrectlyFromFile() {
		BraintreeGateway gateway = BraintreeGatewayFactory
				.fromConfigFile(mockConfigFile);

		String expectedEnvironment = Environment.DEVELOPMENT
				.getEnvironmentName();
		...
		String actualMerchantPath = gateway.getConfiguration()
				.getMerchantPath();
		String actualPrivateKey = gateway.getConfiguration().getPrivateKey();
		String actualEnvironment = gateway.getConfiguration().getEnvironment()
				.getEnvironmentName();
		String actualPublicKey = gateway.getConfiguration().getPublicKey();

		Assert.assertEquals("failure - received unexpected environment",
				expectedEnvironment, actualEnvironment);
		Assert.assertEquals("failure - received unexpected merchantPath",
				expectedMerchantPath, actualMerchantPath);
		Assert.assertEquals("failure - received unexpected publicKey",
				expectedPublicKey, actualPublicKey);
		Assert.assertEquals("failure - received unexpected privateKey",
	...
	public void loadsCorrectlyFromMapping() {
		BraintreeGateway gateway = BraintreeGatewayFactory
				.fromConfigMapping(mockConfigMapping);
	...
	@Test
	public void throwsNullPointerExceptionIfMissingElementsFromFile()
			throws IOException {
		File emptyFile = File.createTempFile("empty.properties", ".tmp");
		Exception exception = null;
		try {
			BraintreeGateway gateway = BraintreeGatewayFactory
					.fromConfigFile(emptyFile);
		} catch (Exception e) {
			exception = e;
		}
	...
	public void logsErrorAndThrowsNullPointerExceptionIfFileNotFound() {
		...
		try {
			...
			BraintreeGateway gateway = BraintreeGatewayFactory
					.fromConfigFile(new File("notARealConfig.properties"));
		} catch (Exception e) {
			exception = e;
		} finally {
	...
	public void throwsNullPointerExceptionIfMissingElementsFromMapping() {
		...
		try {
			BraintreeGateway gateway = BraintreeGatewayFactory
					.fromConfigMapping(emptyMapping);
		} catch (Exception e) {
			exception = e;
		}
